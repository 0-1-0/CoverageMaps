...
public class Voronoi {
	...
	public Voronoi( float[][] points ){
		...
		for(int i=0; i<points.length; i++){
			IntArray faceOrder = new IntArray(pointBuckets[i].getLength());
			
			//TODO: remove this stub
			if(pointBuckets[i].getLength()  <= 0)continue;
			...
			while(p>=0){
				...
				for( int k=0; k<faceNet.get(p).linkCount(); k++ ){
					int neighbor = faceNet.get(p).getLink(k);
			...
			regions[i] = new MPolygon(pointBuckets[i].getLength());
			for( int f=0; f<faceOrder.getLength(); f++ ){
